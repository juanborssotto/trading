...
class DNSAlarmReporterETHRedLines(IStrategy):
    minimal_roi = {
        "0": 10
    }
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if self.dp and \
            ...
            #print(self.df_2h["date"].iloc[-1])
            #print(datetime.now())
            #print(type(self.df_2h["date"].iloc[-1]))
            #print(self.df_2h["date"].iloc[-1].to_pydatetime() > datetime.now())
            #print(datetime.now() > self.df_2h["date"].iloc[-1].to_pydatetime())
    ...
    def calculate_dns(self, dataframe, ticker, pair, timeframe):
        ...
        desktop_notif_text = f'{pair} {timeframe} BUY: {distance_closest_demand_red_line} ' \
               f'SELL: {distance_closest_offer_red_line}'
        text = f'{pair} {timeframe} BUY: {red(distance_closest_demand_red_line)} ' \
               f'SELL: {red(distance_closest_offer_red_line)}'
        if any_under_threshold(pair, distance_closest_demand_red_line):
            ...
            text += "    BUY    "
        if any_under_threshold(pair, distance_closest_offer_red_line):
            text += "    SELL    "
        print(text)
