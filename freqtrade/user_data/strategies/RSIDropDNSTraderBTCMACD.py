...
class RSIDropDNSTraderBTCMACD(IStrategy):
    minimal_roi = {
        "0": 10
    }
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if not trade_manager.has_open_trade:
            if self.should_buy(pair=pair, dataframe=short_df, ongoing_close=ongoing_close,
                               ...
                               btc_macd_hist=self.btc_macd_hist):
                buy_criteria = True
    ...
    def rsi_in_range(self, pair, dataframe, rsi_threshold):
        ...
        for i in range(2, lookback_candles + 1):
            if calculate_percentage_change(last_rsi, rsi[-i]) > rsi_threshold:
                # print(yellow_text(f"{pair} last rsi: {last_rsi} old rsi: {rsi[-i]}"))
                result = True
                break
        return result
    ...
    def should_buy(self, pair, dataframe: DataFrame, ongoing_close: float,
                   closest_bull_green_line: float, closest_bull_red_line: float, drop_rsi_threshold: float, btc_macd_hist) -> bool:
        btc_last_closed_candle_macd_hist = btc_macd_hist[-2]
        btc_current_macd_hist = btc_macd_hist[-1]
        # print(yellow_text(f"{pair} {btc_current_macd_hist} {btc_last_closed_candle_macd_hist}"))
        if closest_bull_green_line and closest_bull_red_line:
            return self.rsi_in_range(pair=pair, dataframe=dataframe, rsi_threshold=drop_rsi_threshold) and \
                   btc_current_macd_hist > btc_last_closed_candle_macd_hist and \
