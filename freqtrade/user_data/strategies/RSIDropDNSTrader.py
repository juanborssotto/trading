...
class RSIDropDNSTrader(IStrategy):
    minimal_roi = {
        "0": 10
    }
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        ongoing_high = short_df['high'].iloc[-1]
        ...
        if not trade_manager.has_open_trade:
            ...
            elif trade_manager.should_profit(ongoing_close=ongoing_high):
                sell_criteria = True
                self.notifications[pair]['notify_profit'] = True
                trade_manager.close_trade()
            elif self.sell_by_bear_zone(dataframe=short_df, ongoing_high=ongoing_high):
                sell_criteria = True
                self.notifications[pair]['notify_profit'] = True
                trade_manager.close_trade()
    ...
    def sell_by_bear_zone(self, dataframe, ongoing_high):
        ...
        if closest_bear_green_line and closest_bear_red_line:
            return closest_bear_green_line >= ongoing_high
        return False
