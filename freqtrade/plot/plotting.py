        ...
        if indicator in data:
            ...
            if row == 1:
                ...
                if indicator == "macd":
                    ...
                    fig.add_trace(scatter, row, 1, True)
                elif indicator == "macdhist":
                    fig.add_trace(scatter, row, 1, True)
                elif indicator == "sma9":
                    fig.add_trace(scatter, row, 1, True)
                elif indicator == "sma15":
                    fig.add_trace(scatter, row, 1, True)
                elif indicator == "rsi":
                    fig.add_trace(scatter, row, 1, True)
                elif indicator == "ema100rate":
                    fig.add_trace(scatter, row, 1, True)
                elif indicator == "volume_condition_datapoint":
                    scatter.mode = 'markers+text'
                    scatter.text = "vol"
                    scatter.textfont = dict(family="sans serif", size=8)
                    fig.add_trace(scatter, row, 1)
                elif indicator == "phase_a_datapoint":
                    scatter.mode = 'markers+text'
                    scatter.text = "phaseA"
                    scatter.textfont = dict(family="sans serif", size=8)
                    fig.add_trace(scatter, row, 1)
                elif indicator == "spring_datapoint":
                    scatter.mode = 'markers+text'
                    scatter.text = "spring"
                    scatter.textfont = dict(family="sans serif", size=8)
                    fig.add_trace(scatter, row, 1)
                elif indicator == "test_datapoint":
                    scatter.mode = 'markers+text'
                    scatter.text = "test"
                    scatter.textfont = dict(family="sans serif", size=8)
                    fig.add_trace(scatter, row, 1)
                elif indicator == "jump_datapoint":
                    scatter.mode = 'markers+text'
                    scatter.text = "jump"
                    scatter.textfont = dict(family="sans serif", size=8)
                    fig.add_trace(scatter, row, 1)
                elif indicator == "phase_a_and_spring_and_test_datapoint":
                    scatter.mode = 'markers+text'
                    scatter.text = "test"
                    scatter.textfont = dict(family="sans serif", size=8)
                    fig.add_trace(scatter, row, 1)
                elif indicator == "support" or indicator == "resistance" or indicator == "phase_a_support" or indicator == "phase_a_resistance" or indicator == "volume_condition_close" or indicator == "volume_condition_open":
                    scatter.mode = 'markers'
                    fig.add_trace(scatter, row, 1)
                elif indicator in ["bull_engulf_green_line", "bear_engulf_green_line"]:
                    scatter.mode = 'markers'
...
def add_shapes_to_row1(fig, data):
    secondary_y = True
    # fig.add_shape(row=1, col=1, type="line",
    #               x0=data.date[0], y0=50, x1=data.date[len(data.date) - 1], y1=50,
    #               line=dict(color="RoyalBlue", width=1), secondary_y=secondary_y)
    # fig.add_shape(row=1, col=1, type="line",
    #               x0=data.date[0], y0=70, x1=data.date[len(data.date) - 1], y1=70,
    #               line=dict(color="RoyalBlue", width=1), secondary_y=True)
    # fig.add_shape(row=1, col=1, type="line",
    #               x0=data.date[0], y0=30, x1=data.date[len(data.date) - 1], y1=30,
    #               line=dict(color="RoyalBlue", width=1), secondary_y=True)
...
def generate_candlestick_graph(pair: str, data: pd.DataFrame, trades: pd.DataFrame = None, *,
    ...
    fig['layout'].update(title=pair, showlegend=True, hovermode='x')
