...
def get_next_value(starting_value, final_value):
        ...
        try:
            ...
            for timeframe in timeframes:
                for pair in pairs:
                    ...
                    # Highs
                    two_candles_back_high = df["high"].iloc[-3]
                    one_candle_back_high = df["high"].iloc[-2]
                    next_high_threshold = get_next_value(two_candles_back_high, one_candle_back_high)
                    ...
                    if -1 < distance_to_next_high_threshold < 1:
                        new_msgs.append(f"{pair} in {timeframe} is close to next high {next_high_threshold}")
                    # Lows
                    two_candles_back_low = df["low"].iloc[-3]
                    one_candle_back_low = df["low"].iloc[-2]
                    next_low_threshold = get_next_value(two_candles_back_low, one_candle_back_low)
                    distance_to_next_low_threshold = (next_low_threshold - ongoing_price) / ongoing_price * 100
                    if -1 < distance_to_next_low_threshold < 1:
                        new_msgs.append(f"{pair} in {timeframe} is close to next low {next_low_threshold}")
