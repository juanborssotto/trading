...
class IStrategy(ABC, HyperStrategyMixin):
    ...
    trades_max_roi_profit: Dict[Trade, float] = defaultdict(list)
...
###
    ...
    def min_roi_reached(self, trade: Trade, current_profit: float, current_time: datetime) -> bool:
        ...
        if "min_roi_reached_strategy" in self.config:
            strategy = self.config["min_roi_reached_strategy"]
            if strategy == "keep_going_no_percentage":
                if trade in self.trades_max_roi_profit:
                    if current_profit >= self.trades_max_roi_profit[trade]:
                        self.trades_max_roi_profit[trade] = current_profit
                        return False
                    else:
                        return True
                ...
                if roi is None:
                    ...
                    if current_profit > roi:
                        self.trades_max_roi_profit[trade] = current_profit
                    return False
            elif strategy == "keep_going_with_percentage":
                percentage = float(self.config["keep_going_with_percentage_value"])
                if trade in self.trades_max_roi_profit:
                    if current_profit > self.trades_max_roi_profit[trade]:
                        self.trades_max_roi_profit[trade] = current_profit
                        return False
                    elif (100 - current_profit * 100 / self.trades_max_roi_profit[trade]) >= percentage:
                        return True
                    return False
                ...
                logger.fatal("invalid min_roi_reached_strategy value")
                exit(1)
        else:
            if roi is None:
                return False
